...
<?php
// WP Offload Media Lite (amazon-s3-and-cloudfront) 配置
// 通过环境变量注入，避免凭证硬编码
if ( ! defined( 'AS3CF_SETTINGS' ) ) {
    $as3cf_endpoint = getenv('S3_ENDPOINT') ?: '';
    $as3cf_region   = getenv('S3_REGION')   ?: 'auto';
    $as3cf_bucket   = getenv('S3_BUCKET')   ?: '';
    $as3cf_key      = getenv('AWS_ACCESS_KEY_ID') ?: '';
    $as3cf_secret   = getenv('AWS_SECRET_ACCESS_KEY') ?: '';
    $as3cf_cdn      = getenv('S3_CDN_DOMAIN') ?: '';

    $as3cf_config = [
        'provider'                => 'aws',
        'access-key-id'           => $as3cf_key,
        'secret-access-key'       => $as3cf_secret,
        'bucket'                  => $as3cf_bucket,
        'region'                  => $as3cf_region,
        'copy-to-s3'              => true,
        'serve-from-s3'           => true,
        ...
        'force-https'             => true,
        'use-object-prefix'       => true,
        'object-prefix'           => 'wp-content/uploads/',
        'enable-object-prefix'    => true,
    ];

    // Cloudflare R2 需要自定义 endpoint 且强制 path style
    if ( $as3cf_endpoint !== '' ) {
        $as3cf_config['use-server-roles']        = false;
        $as3cf_config['endpoint']                = $as3cf_endpoint;
        $as3cf_config['use-path-style-endpoint'] = true;
    }

    // CDN 域名
    if ( $as3cf_cdn !== '' ) {
        $as3cf_config['cloudfront-domain']   = $as3cf_cdn;
        $as3cf_config['enable-delivery-cdn'] = true;
    }

    define( 'AS3CF_SETTINGS', serialize( $as3cf_config ) );
}
PHP
}
...
_setup_plugins() {
    ...
    info "配置 Offload Media 插件（WP Offload Media Lite）..."
    if "${WP[@]}" plugin is-installed ilab-media-tools &>/dev/null; then
        "${WP[@]}" plugin deactivate ilab-media-tools 2>/dev/null || true
        "${WP[@]}" plugin delete ilab-media-tools 2>/dev/null || true
        info "  已卸载 ilab-media-tools"
    fi
    if "${WP[@]}" plugin is-installed amazon-s3-and-cloudfront &>/dev/null; then
        "${WP[@]}" plugin activate amazon-s3-and-cloudfront || warn "Offload Media 激活失败"
    else
        "${WP[@]}" plugin install amazon-s3-and-cloudfront --activate || warn "Offload Media 安装失败"
    fi
...
cmd_master_init() {
    ...
    case "${S3_CHOICE:-1}" in
        2) S3_PROVIDER="cloudflare"
           read -rp "R2 Endpoint URL: " S3_ENDPOINT || true
           [[ -n "$S3_ENDPOINT" ]] || error "必须填写 Endpoint"
           read -rp "区域 [默认: auto]: " S3_REGION || true; S3_REGION="${S3_REGION:-auto}" ;;
        3) S3_PROVIDER="other"
           read -rp "自定义 Endpoint URL: " S3_ENDPOINT || true
           [[ -n "$S3_ENDPOINT" ]] || error "必须填写 Endpoint"
           read -rp "区域 [默认: us-east-1]: " S3_REGION || true; S3_REGION="${S3_REGION:-us-east-1}" ;;
        *) read -rp "区域 [默认: us-east-1]: " S3_REGION || true; S3_REGION="${S3_REGION:-us-east-1}" ;;
    esac
    read -rp "存储桶名称: " S3_BUCKET || true
    [[ -n "$S3_BUCKET" ]] || error "桶名不能为空"
    local S3_KEY="" S3_SECRET=""
    read_secret "S3 Access Key ID: " S3_KEY;     [[ -n "$S3_KEY"    ]] || error "S3 Key 不能为空"
    read_secret "S3 Secret Access Key: " S3_SECRET; [[ -n "$S3_SECRET" ]] || error "S3 Secret 不能为空"
    read -rp "CDN 域名（留空跳过）: " S3_CDN_DOMAIN || true; S3_CDN_DOMAIN="${S3_CDN_DOMAIN:-}"
...
cmd_pull_deploy() {
    ...
    local S3_KEY="" S3_SECRET="" S3_CDN_DOMAIN="" WP_URL="" WP_PORT="80"
